(() => {
  ...
  const CHRISTMAS = { month: 3, day: 28 }; // April 28
  ...
  function initGame(){
    ...
    let W = canvas.width = Math.min(900, innerWidth - 60); let H = canvas.height = 220;
    window.addEventListener('resize', ()=>{ W = canvas.width = Math.min(900, innerWidth - 60); H = canvas.height = 220; setupPads(); });
